...
$(document).ready(function () {
    ...
    // Global Settings
    var fetchActions = "TotalWithDomain,WebRefer";
    ...
    function fetchData() {
        ...
        $.ajax({
            url: "http://" + getServerName(ServerName, ServerPath) + "/vdata1.0/Audience.aspx?rid=" + RegId + "&gid=" + GameId + "&act=" + fetchActions + "&dat=" + startDate + "," + endData + "&pag=" + page + "&dom=" + GetQueryStringParams('dom'),
            method: 'GET',
            dataType: 'json',
        ...
        function onDataReceived(recv) {
            ...
            if ($.inArray("WebRefer", _fetchAction) > -1) {
                ...
                for (myCols in recv.metrics.WebRefer) {
                    htmlBody += ('<tr><td>' + (i + page * pageLimit) + '</td><td><a href="' + recv.metrics.WebRefer[myCols].c1 + '" target="_blank">' + (recv.metrics.WebRefer[myCols].c1).substring(0, 200) + '</a></td><td style="text-align: center;">' + addCommas(recv.metrics.WebRefer[myCols].c2) + '</td><td style="text-align: center;">' + recv.metrics.WebRefer[myCols].c3 + '%</td></tr>');
                    i++;
                }
                $('#tbl_WebRefer').append(htmlBody);
                setPageNumber("#WebReferPager", parseInt(recv.metrics.TotalPage), page, pageLimit);

            }
            if ($.inArray("TotalWithDomain", _fetchAction) > -1) {
