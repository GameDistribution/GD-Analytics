...
$(document).ready(function () {
    ...
    var placeholder = $("#technology-chart");
    ...
    $('#technology-chart').bind("plothover", function (event, pos, item) {
    ...
    // Global Settings
    var fetchActions = "Total,BrowserName";
    ...
    function fetchData() {
        ...
        function onDataReceived(recv) {
            ...
            var htmlBody = '';
            if ($.inArray("BrowserName", _fetchAction) > -1) {
                i = 1;
                $("#tbl_Browsers > tbody").empty();
                for (myCols in recv.metrics.BrowserName) {
                    htmlBody += ('<tr><td>' + (i + page * pageLimit) + '</td><td><a href="TechnologyBrowserDetail.aspx?gid=' + GameId + '&bro=' + encodeURIComponent(recv.metrics.BrowserName[myCols].c1) + '">' + recv.metrics.BrowserName[myCols].c1 + '</a></td><td style="text-align: center;">' + addCommas(recv.metrics.BrowserName[myCols].c2) + '</td><td style="text-align: center;">' + recv.metrics.BrowserName[myCols].c3 + '%</td></tr>');
                    i++;
                }
                $('#tbl_Browsers').append(htmlBody);
