...
$(function () {
    ...
    $("#btnApply").click(function () {
        if ($(".blockedgames").length == 0) {
            bootbox.alert("No web site to block!", function () {
            });
        } else {

            bootbox.dialog({
                message: "Are sure to apply blocking?",
                ...
                buttons: {
                    danger: {
                        ...
                        callback: function () {
                            fetchDoActionData("&act=ApplyBlockGame&gid=" + GameId, onDataReceived);
            ...
            bootbox.dialog({
                ...
                buttons: {
                    danger: {
                        ...
                        className: "btn-danger",
                        callback: function () {
                            fetchDoActionData("&act=DelBlockedGame&gid=" + gid + "&website=" + website, onDataReceived);
                            function onDataReceived(recv) {
                                if (recv == false) {
                                    gotoURL("/");
                                    return;
    ...
    function fetchData() {
        ...
        function onDataReceived(recv) {
            ...
            if ($.inArray("BlockSites", _fetchAction) > -1) {
                i = 1;
                $("#tbl_blocksites > tbody").empty();
                ...
                $row = $(row);
                $("#tbl_blocksites tbody").append(row);
