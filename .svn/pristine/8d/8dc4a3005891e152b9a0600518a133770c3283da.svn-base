...
$(document).ready(function () {
    ...
    // Global Settings
    var fetchActions = "TotalWithOS,OS";
    ...
    function fetchData() {
        ...
        $.ajax({
            url: "http://" + getServerName(ServerName, ServerPath) + "/vdata1.0/Audience.aspx?rid=" + RegId + "&gid=" + GameId + "&act=" + fetchActions + "&dat=" + startDate + "," + endData + "&pag=" + page + "&os=" + GetQueryStringParams('os'),
            method: 'GET',
            dataType: 'json',
        ...
        function onDataReceived(recv) {
            ...
            var htmlBody = '';
            if ($.inArray("OS", _fetchAction) > -1) {
                i = 1;
                $("#tbl_OS > tbody").empty();
                for (myCols in recv.metrics.OS) {
                    htmlBody += ('<tr><td>' + (i + page * pageLimit) + '</td><td>' + (recv.metrics.OS[myCols].c1).substring(0, 200) + '</td><td style="text-align: center;">' + addCommas(recv.metrics.OS[myCols].c2) + '</td><td style="text-align: center;">' + recv.metrics.OS[myCols].c3 + '%</td></tr>');
                    i++;
                }
                $('#tbl_OS').append(htmlBody);
                setPageNumber("#OSPager", parseInt(recv.metrics.TotalPage), page, pageLimit);

            }
            if ($.inArray("TotalWithOS", _fetchAction) > -1) {
