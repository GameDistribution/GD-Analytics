...
$(document).ready(function () {
    ...
    // Global Settings
    var fetchActions = "Total,WebReferDomain";
    ...
    function fetchData() {
        ...
        function onDataReceived(recv) {
            ...
            var htmlBody = '';
            if ($.inArray("WebReferDomain", _fetchAction) > -1) {
                i = 1;
                $("#tbl_WebReferDomain > tbody").empty();
                for (myCols in recv.metrics.WebReferDomain) {
                    htmlBody += ('<tr><td>' + (i + page * pageLimit) + '</td><td><a href="AudienceTrafficRefer.aspx?gid=' + GameId + '&dom=' + encodeURIComponent(recv.metrics.WebReferDomain[myCols].c1) + '">' + recv.metrics.WebReferDomain[myCols].c1 + '</a></td><td style="text-align: center;">' + addCommas(recv.metrics.WebReferDomain[myCols].c2) + '</td><td style="text-align: center;">' + recv.metrics.WebReferDomain[myCols].c3 + '%</td><td style="text-align: center;"><a class="btn btn-xs bansite" href="#" gid="' + GameId + '" website="' + encodeURIComponent(recv.metrics.WebReferDomain[myCols].c1) + '"><i class="glyphicon glyphicon-fire"></i></a></td></tr>');
                    i++;
                }
                $('#tbl_WebReferDomain').append(htmlBody);

                $(".bansite").click(function (e) {
                    e.preventDefault();
                    doJob("&act=AddBlockGame&gid=" + $(this).attr("gid") + "&website=" + $(this).attr("website"), onDataReceived);
                    function onDataReceived(recv) {
                ...
                $(".banads").click(function (e) {
                    e.preventDefault();
                    doJob("&act=AddBlockGameBanner&gid=" + $(this).attr("gid") + "&website=" + $(this).attr("website"), onDataReceived);
                    function onDataReceived(recv) {
                        waitDialog.hidePleaseWait();
                        if (recv.code == 200) {
                            bootbox.alert(recv.message, function () { });
                ...
                setPageNumber("#WebReferDomainPager", parseInt(recv.metrics.TotalPage), page, pageLimit);
    ...
    function doJob(param, onDataReceived) {
        waitDialog.showPleaseWait();
        fetchTimer = Math.round((new Date()).getTime() / 1000);
        $.ajax({
