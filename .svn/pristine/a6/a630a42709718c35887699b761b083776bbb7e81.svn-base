...
$(document).ready(function () {
    ...
    function fetchData() {
        ...
        function onDataReceived(recv) {
            ...
            var htmlBody = '';
            if ($.inArray("Device", _fetchAction) > -1) {
                i = 1;
                $("#tbl_Device > tbody").empty();
                for (myCols in recv.metrics.Device) {
                    htmlBody += ('<tr><td>' + (i + page * pageLimit) + '</td><td>' + recv.metrics.Device[myCols].c1 + '</td><td style="text-align: center;">' + addCommas(recv.metrics.Device[myCols].c2) + '</td><td style="text-align: center;">' + recv.metrics.Device[myCols].c3 + '%</td></tr>');
                    i++;
                }
                $('#tbl_Device').append(htmlBody);

                setPageNumber("#DevicePager", parseInt(recv.metrics.TotalPage), page, pageLimit);
