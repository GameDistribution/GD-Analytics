...
$(document).ready(function () {
    ...
    function fetchData() {
        ...
        function onDataReceived(recv) {
            ...
            var htmlBody = '';
            if ($.inArray("OSName", _fetchAction) > -1) {
                i = 1;
                $("#tbl_OS > tbody").empty();
                for (myCols in recv.metrics.OSName) {
                    htmlBody += ('<tr><td>' + (i + page * pageLimit) + '</td><td><a href="TechnologyOSDetail.aspx?gid=' + GameId + '&os=' + encodeURIComponent(recv.metrics.OSName[myCols].c1) + '">' + recv.metrics.OSName[myCols].c1 + '</a></td><td style="text-align: center;">' + addCommas(recv.metrics.OSName[myCols].c2) + '</td><td style="text-align: center;">' + recv.metrics.OSName[myCols].c3 + '%</td></tr>');
                    i++;
                }
